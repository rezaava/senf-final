    ...
    <div class="col px-3">
        ...
        <div class="row g-0 mt-4 p-3 rounded-4 shadow bg-white pb-3">
            ...
            <form action="{{ route('organ.store') }}" class="px-4 mt-4" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <label for="name" class="form-label">نام سالن<span class="text-danger">*</span> :</label>
                ...
                <!-- contracts -->
                <div class="mb-3">
                    <label for="target_contract" class="form-label">انتخاب قرارداد <span class="text-danger">*</span>
                        :</label>
                    <br>
                    <small class="text-danger">انتخاب قرارداد به منزله قبول شرایط و امضاء قرارداد می‌باشد</small>
                    <select name="contract" id="target_contract" class="form-select">
                        <option value="" selected disabled>انتخاب کنید</option>
                        @foreach ($contracts as $contract)
                            <option value="{{ $contract->id }}" data-text="{{ $contract->text }}"
                                data-percentage="{{ $contract->percentage }}">
                                {{ $contract->title }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <div class="alert alert-secondary mt-3" style="display: none" id="contract_text">
                        <strong>متن قرارداد: </strong><span id="contract_body"></span><br>
                        <strong>درصد قرارداد: </strong><span id="contract_percentage"></span>%
                    </div>
                </div>
                ...
                <div class="row mt-3">
                    ...
                    <div class="col-md-4 mb-3">
                        <label for="category" class="form-label">حوزه سالن <span class="text-danger">*</span> :</label>
...
@section('javaScript')
    ...
    <script>
        $('#target_contract').on('change', function() {
            let selectedOption = $(this).find('option:selected');
            let contractText = selectedOption.data('text');
            let contractPercentage = selectedOption.data('percentage');

            $('#contract_body').text(contractText);
            $('#contract_percentage').text(contractPercentage);
            $('#contract_text').show();
        });
    </script>
@endsection
